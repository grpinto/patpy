"""Click-based CLI entrypoint for patpy-analysis-mcp.

Mirrors the BioContextAI cookiecutter ``main.py`` shape so that running
``uvx patpy-analysis-mcp`` (or ``patpy-analysis-mcp`` after
``pip install``) feels identical to other registry servers. The
``from .tools import *`` import is intentional: it triggers the
side-effect registration of every ``@mcp.tool``-decorated function under
``patpy_analysis_mcp.tools``.
"""
...
@click.option(
    ...
    "show_version",
    is_flag=True,
    help="Print the patpy-analysis-mcp version and exit.",
)
def run_app(
    transport: str,
    port: int,
    ...
    """Run the patpy-analysis-mcp server.
    ...
    """
    if show_version:
        from patpy_analysis_mcp import __version__
    ...
    logger = logging.getLogger("patpy_analysis_mcp")

    from patpy_analysis_mcp.mcp import mcp

    transport_lower = transport.lower()
    logger.info(
        "Starting patpy-analysis-mcp (env=%s, transport=%s)",
        environment.name,
        transport_lower,
    )
